...
try:
    ...
    print >> sys.stderr, 'To build ubuntu-touch-manager you need https://launchpad.net/python-distutils-extra'
    sys.exit(1)
assert DistUtilsExtra.auto.__version__ >= '2.18', 'needs DistUtilsExtra.auto >= 2.18'

def update_config(libdir, values = {}):

    filename = os.path.join(libdir, 'ubuntu_touch_manager_lib/ubuntu_touch_managerconfig.py')
    oldvalues = {}
    try:
        fin = file(filename, 'r')
...
def move_desktop_file(root, target_data, prefix):
    ...
    old_desktop_path = os.path.normpath(root + target_data +
                                        '/share/applications')
    old_desktop_file = old_desktop_path + '/ubuntu-touch-manager.desktop'
    desktop_path = os.path.normpath(root + prefix + '/share/applications')
    desktop_file = desktop_path + '/ubuntu-touch-manager.desktop'
    ...
    if not os.path.exists(old_desktop_file):
        ...
        desktop_file = desktop_path + '/extras-ubuntu-touch-manager.desktop'
        try:
...
def update_desktop_file(filename, target_pkgdata, target_scripts):
    ...
    try:
        ...
        for line in fin:
            if 'Icon=' in line:
                line = "Icon=%s\n" % (target_pkgdata + 'media/ubuntu-touch-manager.svg')
            elif 'Exec=' in line:
                cmd = line.split("=")[1].split(None, 1)
                line = "Exec=%s" % (target_scripts + 'ubuntu-touch-manager')
                if len(cmd) > 1:
...
class InstallAndUpdateDataDirectory(DistUtilsExtra.auto.install_auto):
    def run(self):
        ...
        target_pkgdata = target_data + 'share/ubuntu-touch-manager/'
        target_scripts = '/' + os.path.relpath(self.install_scripts, self.root) + '/'

        values = {'__ubuntu_touch_manager_data_directory__': "'%s'" % (target_pkgdata),
...
DistUtilsExtra.auto.setup(
    name='ubuntu-touch-manager',
    version='0.1.8.4',
    license='GPL-3',
    author='Szymon Waliczek',
    author_email='majsterrr@gmail.com',
    description='Application for managigng your Ubuntu Touch device.',
    url='https://launchpad.net/ubuntu-touch-manager',
    long_description='This application lets you takes screenshots and share them to Ubuntu One or Dropbox, download/upload content from/to your mobile device and more to come... ;-)',
